export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `http://44.206.53.75/Sales-1.0/REST_Index.php/backend/UpdatePrice?username=${req.body.id}&iddpf=${req.body.iddpf}`,
      {
        method: 'post',
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Bad response from server: Edit Payment - ${errorText}`);
    }
