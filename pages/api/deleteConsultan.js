export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `http://44.206.53.75/Sales-1.0/REST_Index.php/backend/deleteSalesConsultant?salesConsultantId=${req.body.salesConsultantId}&username=${req.body.id}`,
      {
        method: 'delete',
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Bad response from server: Delete consultant - ${errorText}`
      );
    }
